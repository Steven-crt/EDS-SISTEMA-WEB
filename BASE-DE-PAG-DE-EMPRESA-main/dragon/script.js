...
let radm = Math.min(width, height) * 0.28;

const resize = () => {
	width = window.innerWidth;
	height = window.innerHeight;
	radm = Math.min(width, height) * 0.28;
};
...
const handlePointerMove = (x, y, now) => {
	const dx = x - user.lastX;
	const dy = y - user.lastY;
...
const handlePointerDown = (x, y, now) => {
	user.lastClick = now;
	const dx = head.x - x;
	const dy = head.y - y;
...
window.addEventListener(
	"pointermove",
	(e) => handlePointerMove(e.clientX, e.clientY, performance.now()),
	false
);

window.addEventListener(
	"pointerdown",
	(e) => handlePointerDown(e.clientX, e.clientY, performance.now()),
	false
);

window.addEventListener("message", (event) => {
	const data = event.data;
	if (!data || data.type !== "dragon-pointer") return;
	if (typeof data.x !== "number" || typeof data.y !== "number") return;
	const now = typeof data.time === "number" ? data.time : performance.now();
	if (data.action === "move") handlePointerMove(data.x, data.y, now);
	if (data.action === "down") handlePointerDown(data.x, data.y, now);
});
...
const updatePetTarget = (now, dt) => {
	...
	const orbitAngle = now * 0.0007 + pet.orbit;
	const orbitBoost = pet.state === "excited" ? 32 : pet.state === "curious" ? 14 : 0;
	const orbitRadius = 70 + 85 * pet.curiosity + orbitBoost;
	const orbitPos = {
		x: user.x + Math.cos(orbitAngle) * orbitRadius,
		y: user.y + Math.sin(orbitAngle * 1.4) * orbitRadius * 0.7
	};
	const wanderRadiusX = width * 0.2 + 30 * pet.energy;
	const wanderRadiusY = height * 0.18 + 24 * pet.energy;
	const wanderPos = {
...
const run = () => {
	...
	let followEase = 0.05 + pet.energy * 0.04;
	if (pet.state === "curious") followEase += 0.02;
	if (pet.state === "excited") followEase += 0.035;
	if (pet.state === "idle") followEase -= 0.015;
	followEase = clamp(followEase, 0.035, 0.14);
	...
	if (rad < radm) rad++;
	frm += 0.0022 + pet.energy * 0.0016 + (pet.state === "excited" ? 0.0022 : 0);
};
